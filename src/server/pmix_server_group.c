...
/* DEFINE A LOCAL COLLECTIVE LIST */
...
static void _grpcbfunc(int sd, short args, void *cbdata)
{
    ...
    pmix_list_remove_item(&grp_collectives, &blk->super);
    PMIX_RELEASE(blk);
...
pmix_output(0, "RECVD ADD MEMBER PARTICIPANT");
...
pmix_output(0, "recvd grpconstruct cmd from %s with %lu procs",
